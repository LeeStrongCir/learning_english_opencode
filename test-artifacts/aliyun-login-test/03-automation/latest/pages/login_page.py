...
class LoginPage:
    ...
    LOCATORS = {
        # 账号密码登录模式
        "account_input": "#fm-login-id",
        "password_input": "#fm-login-password",
        "login_button": "#fm-login-submit",
        "remember_checkbox": "#fm-login-remember",
        "forgot_password_link": "a[href*='forget']",

        # 错误提示
        "error_message": ".login-error-msg, .error-msg, .fm-error, [class*='error']",
        "error_container": "#login-error",

        # 登录方式切换
        "sms_login_tab": "text=短信登录",
        "qr_login_tab": "text=扫码登录",
        "password_login_tab": "text=密码登录",

        # 短信登录
        "phone_input_sms": "#mobile",
        "sms_code_input": "#mobilecode",
        "send_sms_button": "text=获取验证码",
        "sms_login_button": "#mobilelogin-submit",

        # 扫码登录
        "qr_code_image": "img[src*='qr']",
        "qr_code_container": "#alibaba-login-box",
        ...
        # 协议勾选
        "agreement_checkbox": "#login-agreement-checkbox, .agreement-checkbox",
        "service_agreement": "text=服务条款",
        "privacy_policy": "text=隐私政策",

        # 成功跳转后的元素
        "console_url": "https://home.console.aliyun.com",
        "user_menu": ".user-menu, .account-info",
    }
    ...
    def switch_to_qr_login(self) -> None:
        ...
        qr_tab = self.page.locator(self.LOCATORS["qr_login_tab"])
        qr_tab.wait_for(state="visible", timeout=10000)
        qr_tab.click()
        self.page.wait_for_timeout(1000)
