...
import { assert as assert3, ifInBrowser as ifInBrowser2, ifInWorker as ifInWorker2 } from "@midscene/shared/utils";
  ...
  async playTask(taskStatus, agent) {
    ...
    for (const flowItemIndex in flow) {
      ...
      if ("aiAction" in flowItem || "ai" in flowItem) {
        ...
        assert3(prompt, "missing prompt for aiString");
        assert3(
          typeof prompt === "string",
...
import { ifInBrowser as ifInBrowser3, ifInWorker as ifInWorker3 } from "@midscene/shared/utils";
...
var PlaygroundServer = class {
  ...
  saveContextFile(uuid2, context) {
    ...
    writeFileSync3(tmpFile, context);
    return tmpFile;
  }
  async launch(port) {
    ...
    if (this.staticPath) {
      ...
      this.app.get("*", (req, res) => {
        const requestedPath = join3(this.staticPath, req.path);
        if (existsSync3(requestedPath)) {
          res.sendFile(requestedPath);
        } else {
          res.sendFile(join3(this.staticPath, "index.html"));
        }
      });
    }
