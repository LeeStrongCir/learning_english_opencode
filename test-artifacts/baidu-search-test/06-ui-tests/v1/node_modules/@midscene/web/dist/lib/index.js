...
// src/common/agent.ts
var import_utils12 = require("@midscene/core/utils");
...
var TaskCache = class {
  ...
  constructor(cacheId, isCacheResultUsed, cacheFilePath) {
    ...
    const cacheMaxFilenameLength = (0, import_env.getAIConfigInNumber)(import_env.MIDSCENE_CACHE_MAX_FILENAME_LENGTH) || DEFAULT_CACHE_MAX_FILENAME_LENGTH;
    if (Buffer.byteLength(safeCacheId, "utf8") > cacheMaxFilenameLength) {
      const prefix = safeCacheId.slice(0, 32);
      const hash = (0, import_utils4.generateHashId)(void 0, safeCacheId);
      safeCacheId = `${prefix}-${hash}`;
    }
    this.cacheId = safeCacheId;
    this.cacheFilePath = import_utils3.ifInBrowser || import_utils3.ifInWorker ? void 0 : cacheFilePath || (0, import_node_path2.join)((0, import_common2.getMidsceneRunSubDir)("cache"), `${this.cacheId}${cacheFileExt}`);
  ...
  matchCache(prompt, type) {
    for (let i = 0; i < this.cacheOriginalLength; i++) {
      ...
      if (item.type === type && (0, import_node_util.isDeepStrictEqual)(item.prompt, prompt) && !this.matchedCacheIndices.has(key)) {
...
async function parseContextFromWebPage(page, _opt) {
  (0, import_utils7.assert)(page, "page is required");
  ...
  (0, import_utils6.uploadTestInfoToServer)({ testUrl: url });
  ...
  (0, import_utils7.assert)(screenshotBase64, "screenshotBase64 is required");
...
function getReportFileName(tag = "web") {
  ...
  const uniqueId = (0, import_utils7.uuid)().substring(0, 8);
  return `${reportTagName || tag}-${dateTimeInFileName}-${uniqueId}`;
}
function printReportMsg(filepath) {
  (0, import_utils7.logMsg)(`Midscene - report file updated: ${filepath}`);
}
function replaceIllegalPathCharsAndSpace(str) {
  return str.replace(/[:*?"<>| ]/g, "-");
}
function forceClosePopup(page, debug8) {
  page.on("popup", async (popup) => {
    ...
    if (!popup.isClosed()) {
      try {
        ...
        debug8(`failed to close popup ${url}, error: ${error}`);
      }
    } else {
      debug8(`popup is already closed, skip close ${url}`);
    }
    if (!page.isClosed()) {
      try {
        await page.goto(url);
      } catch (error) {
        debug8(`failed to goto ${url}, error: ${error}`);
      }
    } else {
      debug8(`page is already closed, skip goto ${url}`);
    }
  });
}
...
// src/common/tasks.ts
var debug3 = (0, import_logger3.getDebug)("page-task-executor");
...
var PageTaskExecutor = class {
    ...
    const taskWithScreenshot = {
      ...
      executor: async (param, context, ...args) => {
        ...
        if (taskApply.type === "Action") {
          await Promise.all([
            (async () => {
              await (0, import_utils8.sleep)(100);
      ...
      if (plan2.type === "Locate") {
        ...
        const taskActionTap = {
          ...
          executor: async (param, { element }) => {
            (0, import_utils9.assert)(element, "Element not found, cannot tap");
        ...
        const taskActionRightClick = {
          ...
          locate: plan2.locate,
          executor: async (param, { element }) => {
            (0, import_utils9.assert)(element, "Element not found, cannot right click");
        ...
        const taskActionHover = {
          ...
          locate: plan2.locate,
          executor: async (param, { element }) => {
            (0, import_utils9.assert)(element, "Element not found, cannot hover");
        ...
        const taskActionScroll = {
          ...
          executor: async (taskParam, { element }) => {
            ...
            if (scrollToEventName === "untilTop") {
              ...
              await (0, import_utils8.sleep)(500);
            } else {
              throw new Error(
        ...
        const taskActionSleep = {
          ...
          locate: plan2.locate,
          executor: async (taskParam) => {
            await (0, import_utils8.sleep)(taskParam?.timeMs || 3e3);
          }
        };
  ...
  async action(userPrompt, actionContext, opts) {
    ...
    const logList = [];
    const yamlFlow = [];
    const replanningCycleLimit = (0, import_env3.getAIConfigInNumber)(import_env3.MIDSCENE_REPLANNING_CYCLE_LIMIT) || defaultReplanningCycleLimit;
    while (planningTask) {
  ...
  async waitFor(assertion, opt) {
    ...
    (0, import_utils9.assert)(assertion, "No assertion for waitFor");
    (0, import_utils9.assert)(timeoutMs, "No timeoutMs for waitFor");
    (0, import_utils9.assert)(checkIntervalMs, "No checkIntervalMs for waitFor");
...
var import_utils11 = require("@midscene/shared/utils");
var debug4 = (0, import_logger4.getDebug)("plan-builder");
function buildPlans(type, locateParam, param) {
  ...
  if (type === "Tap" || type === "Hover" || type === "RightClick") {
    (0, import_utils11.assert)(locateParam, `missing locate info for action "${type}"`);
    (0, import_utils11.assert)(locatePlan, `missing locate info for action "${type}"`);
    const tapPlan = {
      type,
      param: null,
      thought: "",
  ...
  if (type === "Input" || type === "KeyboardPress") {
    ...
    (0, import_utils11.assert)(param, `missing param for action "${type}"`);
    const inputPlan = {
      type,
      param,
      thought: "",
...
// src/common/agent.ts
var debug5 = (0, import_logger5.getDebug)("web-integration");
...
var PageAgent = class {
  constructor(page, opts) {
    ...
    if (opts?.cacheId && this.page.pageType !== "android") {
      this.taskCache = new TaskCache(
        opts.cacheId,
        (0, import_env4.getAIConfigInBoolean)("MIDSCENE_CACHE")
  ...
  async getUIContext(action) {
    ...
    return await parseContextFromWebPage(this.page, {
      ignoreMarker: !!(0, import_env4.vlLocateMode)()
    });
  }
  ...
  dumpDataString() {
    ...
    return (0, import_utils12.stringifyDumpData)(this.dump);
  }
  reportHTMLString() {
    return (0, import_utils12.reportHTMLContent)(this.dumpDataString());
  }
  writeOutActionDumps() {
    if (this.destroyed) {
      throw new Error(
    ...
    this.reportFile = (0, import_utils12.writeLogFile)({
      fileName: this.reportFileName,
      fileExt: import_utils12.groupedActionDumpFileExt,
...
${errorTask?.errorStack}`, {
  ...
  buildDetailedLocateParam(locatePrompt, opt) {
    (0, import_utils13.assert)(locatePrompt, "missing locate prompt");
  ...
  async aiInput(value, locatePrompt, opt) {
    (0, import_utils13.assert)(
      typeof value === "string",
      "input value must be a string, use empty string if you want to clear the input"
    );
    (0, import_utils13.assert)(locatePrompt, "missing locate prompt for input");
    const detailedLocateParam = this.buildDetailedLocateParam(
      locatePrompt,
      opt
    );
  ...
  async aiKeyboardPress(keyName, locatePrompt, opt) {
    (0, import_utils13.assert)(keyName, "missing keyName for keyboard press");
  ...
  async aiAction(taskPrompt, opt) {
    ...
    const isVlmUiTars = (0, import_env4.vlLocateMode)() === "vlm-ui-tars";
  ...
  async describeElementAtPoint(center, opt) {
    ...
    while (!success && retryCount < retryLimit) {
      ...
      (0, import_utils13.assert)(text.description, `failed to describe element at [${center}]`);
...
var import_constants4 = require("@midscene/shared/constants");

// src/puppeteer/base-page.ts
var import_utils17 = require("@midscene/core/utils");
...
var debugPage = (0, import_logger6.getDebug)("web:page");
var Page = class {
  ...
  async getElementsNodeTree() {
    ...
    (0, import_utils18.assert)(scripts, "scripts should be set before writing report in browser");
    const startTime = Date.now();
  ...
  async clearInput(element) {
    ...
    const backspace = async () => {
      await (0, import_utils17.sleep)(100);
...
var import_constants5 = require("@midscene/shared/constants");
var import_logger7 = require("@midscene/shared/logger");
var import_test = require("@playwright/test");
var debugPage2 = (0, import_logger7.getDebug)("web:playwright:ai-fixture");
...
var PlaywrightAiFixture = (options) => {
  const {
    ...
    waitForNetworkIdleTimeout = import_constants5.DEFAULT_WAIT_FOR_NETWORK_IDLE_TIMEOUT,
    waitForNavigationTimeout = import_constants5.DEFAULT_WAIT_FOR_NAVIGATION_TIMEOUT
  } = options ?? {};
  const pageAgentMap = {};
...
var debug6 = (0, import_logger8.getDebug)("playwright:agent");
...
var import_logger9 = require("@midscene/shared/logger");

// src/puppeteer/page.ts
var import_constants6 = require("@midscene/shared/constants");
var WebPage2 = class extends Page {
  constructor(page, opts) {
    super(page, "puppeteer", opts);
    const {
      waitForNavigationTimeout = import_constants6.DEFAULT_WAIT_FOR_NAVIGATION_TIMEOUT,
      waitForNetworkIdleTimeout = import_constants6.DEFAULT_WAIT_FOR_NETWORK_IDLE_TIMEOUT
    } = opts ?? {};
    this.waitForNavigationTimeout = waitForNavigationTimeout;
    this.waitForNetworkIdleTimeout = waitForNetworkIdleTimeout;
  }
  async waitUntilNetworkIdle(options) {
    ...
    await this.underlyingPage.waitForNetworkIdle({
      ...
      concurrency: options?.concurrency ?? import_constants6.DEFAULT_WAIT_FOR_NETWORK_IDLE_CONCURRENCY,
...
var debug7 = (0, import_logger9.getDebug)("puppeteer:agent");
var PuppeteerAgent = class extends PageAgent {
  constructor(page, opts) {
    const webPage = new WebPage2(page, opts);
    super(webPage, opts);
    const { forceSameTabNavigation = true } = opts ?? {};
    if (forceSameTabNavigation) {
      forceClosePopup(page, debug7);
    }
  }
};
